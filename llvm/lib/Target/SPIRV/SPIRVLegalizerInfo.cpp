...
SPIRVLegalizerInfo::SPIRVLegalizerInfo(const SPIRVSubtarget &ST) {
  ...
  const LLT p6 = LLT::pointer(6, PSize); // Input
  const LLT p7 = LLT::pointer(7, PSize); // Input
  const LLT p8 = LLT::pointer(8, PSize); // Input
  const LLT p9 = LLT::pointer(9, PSize); // Input
  const LLT p10 = LLT::pointer(10, PSize); // Input
  ...
  auto allPtrsScalarsAndVectors = {
      ...
      s1,    s8,    s16,   s32,   s64,
      v2s1,  v2s8,  v2s16, v2s32, v2s64, v3s1,   v3s8,
  ...
  auto allWritablePtrs = {p0, p1, p3, p4, p6, p8, p9, };
